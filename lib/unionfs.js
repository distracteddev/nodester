(function() {
  ...
  unionfs.prototype.start = function(cb) {
    ...
    log.info(this.path_base, this.path_app, this.path_rw, this.target_path);
    log.info(this.tmp_base, this.tmp_base_os, this.tmp_base_app, this.tmp_base_app_app, this.tmp_base_rw);
        ...
        lib.ensure_mounted(this.path_app, this.tmp_base_app_app, '-o ro --bind', function(respB) {
          if (respB === true) {
            lib.ensure_mounted(this.path_rw, this.tmp_base_rw, '-o rw --bind', function(respC) {
              if (respC === true) {
                exec('echo nodester | sudo -S unionfs-fuse -o ' + unionfs_opts + ' /rw=RW:/app=RO:/os=RO ' + this.target_path, function(err, stdout, stderr) {
		  log.info(err);
                  log.info(stderr);
                  log.info(stdout);
