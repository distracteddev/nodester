          ...
          tearDown(configData.chroot_base, configData.apphome, configData.apprwfolder, configData.appchroot, function (resp) {
            setupUnionFS(configData.chroot_base, configData.apphome, configData.apprwfolder, configData.appchroot, function (resp) {
              if (resp === true) {
                ...
                var sudoPath = path.join(cfg.app_dir, 'scripts', 'chroot_runner.js');
                log.info('starting chroot process with %s', sudoPath);
              
                var forever = require('forever');
                var child = forever.start(['sudo', sudoPath], {
                  silent: true,
                  cwd: configData.appchroot,
                  max: 2,
                  uid: configData.userid,
                  pidFile: path.join(configData.apprwfolder, '.nodester', 'pids', 'runner.pid'),
                  env: {
                    app_port: parseInt(configData.port, 10),
                    app_host: configData.ip,
                    NODE_ENV: 'production'
                  },
                  spawnWith: {
                    env: {
                ...
                child.on('error', function(err) {
                  log.info("FOREVER ERROR: ", err);
                });

                child.on('start', function(err, process) {
                  var fileRunner = path.join(configData.apprwfolder, '.nodester', 'pids', 'runner.pid');
                  log.info("Process Started", fileRunner, process);
                  if (path.existsSync(fileRunner)) { fs.unlinkSync(fileRunner); }
                  fs.writeFileSync(fileRunner, process.pid.toString());
                  setTimeout(function () {
                    var tapp = {
                      pid     : 'unknown',
                      running : 'failed-to-start'
                    };
                    console.log("BEOFRE", tapp);
                    fs.readFile(fileRunner, function (err, pids) {
                    console.log("READING", pids && pids.toString());
